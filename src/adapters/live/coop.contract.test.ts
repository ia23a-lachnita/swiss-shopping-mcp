...
async function createCache(): Promise<FileTtlCache> {
  const directory = await mkdtemp(join(tmpdir(), 'swiss-shopping-mcp-coop-contract-'));
  cacheDirectories.push(directory);
  return new FileTtlCache(directory);
}

describe.skipIf(process.env.RUN_CONTRACT_TESTS !== '1')('Coop contract', () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it('search endpoint returns valid JSON with products', async () => {
    const adapter = new CoopLiveAdapter({ cache: await createCache() });
    ...
    expect(result.data[0]).toHaveProperty('chain', 'coop');
