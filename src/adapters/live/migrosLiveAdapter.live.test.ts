...
async function createCache(): Promise<FileTtlCache> {
  const directory = await mkdtemp(join(tmpdir(), 'swiss-shopping-mcp-migros-live-'));
  cacheDirectories.push(directory);
  return new FileTtlCache(directory);
}

describe.skipIf(process.env.LIVE_SOURCE_TESTS !== '1')('MigrosLiveAdapter live smoke', () => {
  afterEach(async () => {
    await Promise.all(cacheDirectories.splice(0).map((directory) => rm(directory, { recursive: true, force: true })));
  });

  it('searches Migros live source for a known product term', async () => {
    const adapter = new MigrosLiveAdapter({
  ...
  it('finds Migros stores near Zürich', async () => {
    const adapter = new MigrosLiveAdapter({
      cache: await createCache(),
      sourceClient: new SourceHttpClient({ rateLimitPerHostMs: 1_000 }),
    });
